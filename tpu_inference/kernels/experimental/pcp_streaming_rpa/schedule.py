...
@dataclasses.dataclass(frozen=True)
class PcpStreamingScheduleTemplate:
    """Host template for a streaming schedule with logical page indices.

    The static schedule fields only depend on request lengths and PCP layout.
    The physical KV cache page ids still come from the runtime block table. This
    template records the packed schedule plus the exact flattened locations that
    must be patched from the runtime block table.
    """

    packed_schedule: np.ndarray
    actual_steps: np.ndarray
    global_actual_steps: np.ndarray
    patch_fields: tuple[int, ...]
    patch_flat_indices: tuple[np.ndarray, ...]
    patch_req_ids: tuple[np.ndarray, ...]
    patch_logical_page_indices: tuple[np.ndarray, ...]
...
def _build_schedule_template_patch_data(
    packed_schedule: np.ndarray,
    page_size: int,
    kv_pages_per_block: int,
) -> tuple[tuple[int, ...], tuple[np.ndarray, ...], tuple[np.ndarray, ...],
           tuple[np.ndarray, ...]]:
    req_ids = packed_schedule[..., ScheduleField.REQ_ID]
    kv_valid_len = packed_schedule[..., ScheduleField.KV_VALID_LEN]
    patch_fields: list[int] = []
    patch_flat_indices: list[np.ndarray] = []
    patch_req_ids: list[np.ndarray] = []
    patch_logical_page_indices: list[np.ndarray] = []
    row_count = int(np.prod(packed_schedule.shape[:-1]))

    def add_field(field: int, page_offset: int) -> None:
        valid = np.logical_and(req_ids >= 0, kv_valid_len
                               > page_offset * page_size)
        flat_indices = np.flatnonzero(valid.reshape(row_count))
        if flat_indices.size == 0:
            return
        flat_field = packed_schedule[..., field].reshape(row_count)
        patch_fields.append(field)
        patch_flat_indices.append(flat_indices.astype(np.int64, copy=False))
        patch_req_ids.append(
            req_ids.reshape(row_count)[flat_indices].astype(np.int64,
                                                            copy=False))
        patch_logical_page_indices.append(flat_field[flat_indices].astype(
            np.int64, copy=False))

    add_field(ScheduleField.KV_PAGE_IDX, 0)
    if kv_pages_per_block > 1:
        for page_offset in range(kv_pages_per_block):
            add_field(ScheduleField.KV_PAGE_INDICES_START + page_offset,
                      page_offset)

    return (
        tuple(patch_fields),
        tuple(patch_flat_indices),
        tuple(patch_req_ids),
        tuple(patch_logical_page_indices),
    )


def generate_pcp_streaming_schedule_template(
    kv_lens: list[int] | np.ndarray,
    cu_q_lens: list[int] | np.ndarray,
    q_start_offsets: list[int] | np.ndarray,
    block_tables: np.ndarray,
    ...
    """Generate a reusable schedule template with logical page ids.

    The returned packed schedule has the same static fields as
    generate_pcp_streaming_schedule, but page-id fields contain logical indices
    into a per-request source block table. At runtime, call
    materialize_pcp_streaming_schedule_template to patch those fields with the
    physical page ids from the actual block table.
    """
    block_tables = np.asarray(block_tables, dtype=np.int32)
    if block_tables.ndim != 2:
        raise ValueError("block_tables must be a 2D array.")
    logical_block_tables = np.broadcast_to(
        np.arange(block_tables.shape[1], dtype=np.int32),
        block_tables.shape,
    )
    ...
    fields, flat_indices, req_ids, logical_page_indices = (
        _build_schedule_template_patch_data(schedule.packed_schedule,
                                            page_size, kv_pages_per_block))
    return PcpStreamingScheduleTemplate(
        packed_schedule=schedule.packed_schedule,
        actual_steps=schedule.actual_steps,
        global_actual_steps=schedule.global_actual_steps,
        patch_fields=fields,
        patch_flat_indices=flat_indices,
        patch_req_ids=req_ids,
        patch_logical_page_indices=logical_page_indices,
    )


def materialize_pcp_streaming_schedule_template(
    template: PcpStreamingScheduleTemplate,
    block_tables: np.ndarray,
) -> np.ndarray:
    """Patch a logical schedule template with runtime physical page ids."""
    block_tables = np.asarray(block_tables, dtype=np.int32)
    if block_tables.ndim != 2:
        raise ValueError("block_tables must be a 2D array.")
    packed = template.packed_schedule.copy()
    flat = packed.reshape(-1, packed.shape[-1])
    for field, flat_indices, req_ids, logical_page_indices in zip(
            template.patch_fields,
            template.patch_flat_indices,
            template.patch_req_ids,
            template.patch_logical_page_indices,
            strict=True):
        flat[flat_indices, field] = block_tables[req_ids, logical_page_indices]
    return packed
